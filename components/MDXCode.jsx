...
const CopyButton = ({ value }) => {
  ...
  return (
    <Button variant="solid" bg="brand.red" w="3.5rem" size="xs" borderRadius="md" color="white" _hover={{ bg: "brand.red100" }} onClick={onCopy}>
    ...
    <Box>
      ...
      <Highlight {...defaultProps} theme={nightOwl} code={children} language={language}>
        {({ className, style, tokens, getLineProps, getTokenProps }) => (
          <pre className={className} style={{ ...style, fontSize: "0.8rem", background: "none" }}>
            {tokens.map((line, i) => (
